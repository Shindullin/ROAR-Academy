...
x_bias = 0
y_bias=2
def toy_2D_samples(x_bias ,y_bias, linearSeparableFlag):
    if linearSeparableFlag:
        samples1 = np.random.multivariate_normal([5+x_bias, 0+y_bias], [[1, 0],[0, 1]], 100)
        samples2 = np.random.multivariate_normal([-5+x_bias, 0+y_bias], [[1, 0],[0, 1]], 100)
...
samples, labels = toy_2D_samples(x_bias ,y_bias, linearSeparableFlag)
